...
function find_and_add_entries()
    ...
    local has_audio
    local is_image
    if o.audio_no_images then
        has_audio = EXTENSIONS_AUDIO[string.lower(get_extension(filename))]
        is_image = EXTENSIONS_IMAGES[string.lower(get_extension(filename))]
    end
    ...
    if (pl_count > 1 and autoloaded == nil) or
       (pl_count == 1 and EXTENSIONS[string.lower(get_extension(filename))] == nil) then
        if o.audio_no_images and not has_audio then
            for k, v in ipairs(pl) do
                if not string.match(v.filename, "^%.") then
                    local ext = get_extension(v.filename)
                    if ext ~= nil then
                        if EXTENSIONS_AUDIO[string.lower(ext)] then
                            has_audio = true
                        end
                    end
                end
        ...
        if has_audio then
            for i = 1, #pl do
                local ext = string.lower(get_extension(pl[#pl + 1 - i].filename))
                if not EXTENSIONS_AUDIO[ext] and not EXTENSIONS_VIDEO[ext] then
                    if not pl[#pl + 1 - i].current or pl_current ~= pl_count then
                        mp.commandv("playlist-remove", #pl - i)
                    else
                        remove_last = true
                    end
                end
    ...
    table.filter(files, function (v, k)
        ...
        if o.audio_no_images and has_audio and v ~= filename and EXTENSIONS_IMAGES[string.lower(ext)] then
            return false
        end
    ...
    if is_image and has_audio then
        if next(append[1]) then
...
function reset(k, v)
    local pl_count = mp.get_property_number("playlist-count", 1)
    if remove_last and v ~= pl_count then
        mp.commandv("playlist-remove", pl_count - 1)
        remove_last = false
    end
end

mp.observe_property("playlist-pos-1", "number", reset)
